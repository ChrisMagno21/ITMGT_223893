...
def shift_by_letter(letter, letter_shift):
    ...
    # Replace `pass` with your code.
    # Stay within the function. Only use the parameters as input. The function should return your answer.
letter = str(input("Enter letter here:").upper())
letter_shift = str(input("Enter second letter here:").upper())
def shift_by_letter(letter, letter_shift):
    if letter == ' ':
        return ' '

    shift_value = ord(letter_shift) - ord('A')
    new_position = (ord(letter) - ord('A') + shift_value) % 26
...
def vigenere_cipher(message, key):
    ...
    # Replace `pass` with your code.
    # Stay within the function. Only use the parameters as input. The function should return your answer.
message = str(input("Enter letter here:").upper())
key = str(input("Enter letter here:").upper())
def vigenere_cipher(message, key):

    key_stretch = key * ((len(message) // len(key)) + 1)
    key_stretch = key_stretch[:len(message)]
    result = ""
    
    for i in range(len(message)):
        char = message[i]
        key_char = key_stretch[i]

        if 'A' <= char <= 'Z':
            shift_value = ord(key_char) - ord('A')
            new_position = (ord(char) - ord('A') + shift_value) % 26
            result += chr(ord('A') + new_position)
        elif char == ' ':
            result += char
        else:
...
def scytale_cipher(message, shift):
    ...
    # Stay within the function. Only use the parameters as input. The function should return your answer.
message = str(input("Enter letter here:").upper())
shift = int(input("Enter second letter here:"))
def scytale_cipher(message, shift):
...
def scytale_decipher(message, shift):
    ...
    row = [''] * shift
    # Distribute the characters of the message onto the rows
    for i in range(len(message)):
        position = i % shift
        row[position] += message[i]
        #print(message[i],"(",position,")") -> These were to help me visualize what was going on with the code
    #print(row) -> These were to help me visualize what was going on with the code

    # Decipher message by iterating through the variable 'row'
    deciphered_message = ''
    for x in row:
